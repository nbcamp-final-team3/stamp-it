...
extension FirestoreManager {
    ...
    func fetchMissions(assignedTo userId: String, fromGroup groupId: String) -> Observable<[MissionFirestore]> {
        return Observable.create { observer in
            ...
            // 2) 오늘 + 6일까지 계산
            guard let endDate = Calendar.current.date(byAdding: .day, value: 6, to: startOfToday) else {
                observer.onNext([])
                return Disposables.create()
            }
            ...
            let listener = self.missionsCollection(groupId: groupId)
                ...
                .whereField("dueDate", isGreaterThanOrEqualTo: Timestamp(date: startOfToday))
